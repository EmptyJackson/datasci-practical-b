...
class SmoothTargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for label in self.labels:
            self.label_means[label] = (np.sum(y[X == label]) + self.mean * self.mean_samples) / (len(y[X == label]) + self.mean_samples)
        return self
...
class ModalImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.nan_val:
            self.imputer.fit(X[self.feature].replace(self.nan_val, np.NaN).values.reshape(-1, 1))
        else:
            self.imputer.fit(X[self.feature].values.reshape(-1, 1))
        return self
    def transform(self, X, y=None):
        if self.nan_val:
            X[self.feature] = self.imputer.transform(X[self.feature].replace(self.nan_val, np.NaN).values.reshape(-1, 1))
        else:
            X[self.feature] = self.imputer.transform(X[self.feature].values.reshape(-1, 1))
        return X
...
class KNNImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        X_not_na = X.copy()
        if self.nan_val:
            X_not_na[self.feature] = X_not_na[self.feature].replace(self.nan_val, np.NaN)
        X_not_na = X_not_na.dropna(subset=[self.feature]).drop(self.excluded_feats, axis=1)
    ...
    def transform(self, X, y=None):
        ...
        feat_preds = self.knn.predict(predictors) #reshape(-1, 1) may be needed
        pred_i = 0
...
class UnhelpfulAttributeRemover(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.unhelpful_attributes = ['weight', 'payer_code', 'encounter_id', 'patient_nbr']
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
...
class AgeIntervalConverter(BaseEstimator, TransformerMixin):
    def __init__(self):
        ...
        for i in range(1, 10):
            self.range_midpoint['[' + str(i) + '0-' + str(i+1) + '0)'] = i * 10 + 5
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
...
class NumericalTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, truncate_inpatient=True, incl_total_visits=True, drop_visits=False):
        ...
        self.visit_cats = ['number_emergency', 'number_inpatient', 'number_outpatient']
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
        ...
        if self.truncate_inpatient:
            X['number_inpatient'] = X['number_inpatient'].where(X['number_inpatient'] < 5, 5)
...
class RaceEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.race_enc = OneHotEncoder(categories=[self.races], handle_unknown='ignore')
...
class A1CEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.ac_enc = OneHotEncoder(categories=[self.ac_cats], handle_unknown='ignore')
        self.ac_enc.fit(X['A1Cresult'].values.reshape(-1, 1))
        return self
    def transform(self, X, y=None):
        if not self.drop:
            ac = self.ac_enc.transform(X['A1Cresult'].values.reshape(-1, 1)).toarray()
            X = X.join(pd.DataFrame(ac, columns=["A1Cresult_"+cat for cat in self.ac_cats]))
...
class GlucoseEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.glu_enc = OneHotEncoder(categories=[self.glu_cats], handle_unknown='ignore')
        self.glu_enc.fit(X['max_glu_serum'].values.reshape(-1, 1))
        return self
    def transform(self, X, y=None):
        if not self.drop:
            glu = self.glu_enc.transform(X['max_glu_serum'].values.reshape(-1, 1)).toarray()
            X = X.join(pd.DataFrame(glu, columns=["max_glu_serum_"+cat for cat in self.glu_cats]))
...
class MedicalSpecialtyEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        if not self.drop:
            ...
            med_spec_onehot = med_spec_onehot[[spec for spec in self.common_specs if spec in med_spec_onehot.columns]]
...
class AdmissionDischargeEncoder(BaseEstimator, TransformerMixin):
    def __init__(self, threshold=100, include_11=True):
        self.threshold = threshold
        self.id_types = ['admission_type_id', 'admission_source_id', 'discharge_disposition_id']
    ...
    def transform(self, X, y=None):
        for i, type_ in enumerate(self.id_types):
            ...
            type_onehot = type_onehot[[type_ + '_' + str(id_) for id_ in self.common_ids[i]]]
...
class MedicationEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.medication_labels = [med for med in self.medication_labels if med not in self.unused_meds]
        return self
    def transform(self, X, y=None):
...
class DiagIDCBin(BaseEstimator, TransformerMixin):
    def __init__(self, only_first=True):
        ...
        genit = 7
        neo = 8
        self.bin_labels = {1:'diabetes', 2:'circulatory', 3:'respiratory', 4:'digestive', 5:'injury', 6:'musculoskeletal', 7:'genitourinary', 8:'neoplasms'}

        self.max_id = 8
        self.idc_bins = {250:diab, 785:circ, 786:resp, 787:digest, 788:genit}
...
pipeline = Pipeline([
    ...
    ('numerical', NumericalTransformer(truncate_inpatient=False, incl_total_visits=False, drop_visits=False)),
    ('medication info', BinarizeMedicationInfo()),
    ('gender', GenderEncoder()),
    ('admission & discharge', AdmissionDischargeEncoder(threshold=50)),
    ('medication', MedicationEncoder(discard_threshold=50, up_down_threshold=10000)),
    ...
    ('speciality encode', MedicalSpecialtyEncoder(drop=False, include_missing=False, threshold=50)),
    ('glucose impute', ModalImputer('max_glu_serum', nan_val='None')),
    ('glucose encode', GlucoseEncoder(drop=False, include_missing=False, drop_norm=True)),
